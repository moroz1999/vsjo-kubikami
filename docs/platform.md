...
- The project is built with `sjasmplus`: `compile.bat` runs `php tools/map-viewer/generate_asm.php --project` to regenerate route asm and `tools/map-viewer/routes-data.js` from split route JSON data under `routes/`, then runs `php pack_rooms.php` before `_sjasmplus\\sjasmplus.exe main.a80`, regenerating packed `rooms/*.zx0` from editable `rooms_unpacked/*.atr`.
