...
- IM 2 uses only the normal 48K address space and does not switch RAM banks. The runtime stack starts at `#FDFD` and grows downward; the `JP` dispatch occupies `#FDFD..#FDFF`, and the 257-byte vector table occupies `#FE00..#FF00`. After loader handoff, the screen-attribute buffer reuses `#5B00..#5DFF`, and the mutable-room buffer occupies `#F300..#F5BF`. The disk loader resides at `#F600` during loading and is unused afterward. `main.a80` asserts all boundaries.
