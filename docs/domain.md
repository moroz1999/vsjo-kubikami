...
- Room entry must not advance timed room simulation: edge movement commits the hero's wrapped local coordinate only after `goto_*_room` returns, so a nested elevator update can move a rider from the stale edge coordinate and skip a room. Room entry may synchronize derived coordinates, while animation, elevator, and enemy movement advance only from the main game loop.
