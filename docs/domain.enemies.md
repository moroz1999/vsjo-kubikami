...
- Each accepted enemy-damage event queues `sounds.event_enemy_hit`; AY modes map it to zero-based AYFX effect `0` (editor number `1`), and GS modes map it to medium-priority fixed GS sample number `2` loaded from the 22050 Hz `sfx/gs/attack.raw` at note `65`.
